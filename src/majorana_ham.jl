...
A sum of [`FLOYao.MajoranaTerm`](@ref)s to be used as a more performant replacement
of `op::Yao.AbstractBlock` in [`expect`](@ref) and `expect'`

`Yao.AbstractBlock`s that are sums of (scaled) tensor products of Pauli gates
can be directly converted to `MajoranaSum`s:
...
function MajoranaSum(op::AbstractBlock)
    # The yaoblock2majoranasum function is implemented later
    return yaoblock2majoranasum(parameters_eltype(op), op)
end
