...
"""
    majorana_expect(block::AbstractMatrix, reg::MajoranaReg)
...
function majorana_expect(block::AbstractMatrix, locs, reg::MajoranaReg)
    ...
    copyto!(fullH, CartesianIndices((matlocs, matlocs)), block, CartesianIndices(block))

    expval = sum(1:nqubits(reg), init=zero(eltype(reg.state))) do i
        @views reg.state[:,2i] ⋅ (fullH * reg.state[:,2i-1]) - reg.state[:,2i-1] ⋅ (fullH * reg.state[:,2i])
    end

    offset_expval = sum(1:nqubits(reg), init=zero(eltype(reg.state))) do i
        @views - reg.state[:,2i] ⋅ (fullH * reg.state[:,2i]) - reg.state[:,2i-1] ⋅ (fullH * reg.state[:,2i-i])
    end |> imag

    return (- expval - offset_expval) / 4
end

function majorana_expect(block::AbstractMatrix, reg::MajoranaReg)
    expval = sum(1:nqubits(reg), init=zero(eltype(reg.state))) do i
...
for BT in [:AbstractAdd, :(KronBlock{2}), :(PutBlock{2,1,ZGate})]
    @eval function Yao.expect(op::$BT, reg::MajoranaReg)
        ...
        return majorana_expect(majoranaham, reg)
    end
end
